...
public class AcessoDenunciaService {
    ...
    private DenunciaResponseDTO converterDenunciaEmDTO(Denuncia denuncia){
        ...
        List<MensagemDenuncia> mensagensDescriptografadas = denuncia.getMensagens().stream()
                .map(mensagem -> {
                    String mensagemDescriptografada = mensagemDenunciaService.descriptografarMensagem(mensagem.getMensagem());
                    mensagem.setMensagem(mensagemDescriptografada);
                    return mensagem;
                })
                .toList();
